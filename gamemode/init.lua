...
CreateConVar("dec_roundtime", "600", bit.band(FCVAR_ARCHIVE , FCVAR_REPLICATED , FCVAR_NOTIFY))
CreateConVar("dec_runspeed", "310",  bit.band(FCVAR_ARCHIVE , FCVAR_REPLICATED , FCVAR_NOTIFY))
CreateConVar("dec_startmoney_civilian", "200",  bit.band(FCVAR_ARCHIVE , FCVAR_REPLICATED , FCVAR_NOTIFY))
CreateConVar("dec_startmoney_agent", "350",  bit.band(FCVAR_ARCHIVE , FCVAR_REPLICATED , FCVAR_NOTIFY))
CreateConVar("dec_startmoney_vip", "500",  bit.band(FCVAR_ARCHIVE , FCVAR_REPLICATED , FCVAR_NOTIFY))
CreateConVar("dec_income_civ_time", "2",  bit.band(FCVAR_ARCHIVE , FCVAR_REPLICATED , FCVAR_NOTIFY))
CreateConVar("dec_income_civ_amount", "1",  bit.band(FCVAR_ARCHIVE , FCVAR_REPLICATED , FCVAR_NOTIFY))
CreateConVar("dec_income_agent_time", "1",  bit.band(FCVAR_ARCHIVE , FCVAR_REPLICATED , FCVAR_NOTIFY))
CreateConVar("dec_income_agent_amount", "1",  bit.band(FCVAR_ARCHIVE , FCVAR_REPLICATED , FCVAR_NOTIFY))
CreateConVar("dec_income_vip_time", "1",  bit.band(FCVAR_ARCHIVE , FCVAR_REPLICATED , FCVAR_NOTIFY))
CreateConVar("dec_income_vip_amount", "2",  bit.band(FCVAR_ARCHIVE , FCVAR_REPLICATED , FCVAR_NOTIFY))
CreateConVar("dec_startmoney_vip", "500",  bit.band(FCVAR_ARCHIVE , FCVAR_REPLICATED , FCVAR_NOTIFY))
CreateConVar("dec_rounds", "7",  bit.band(FCVAR_ARCHIVE , FCVAR_REPLICATED , FCVAR_NOTIFY))
CreateConVar("dec_healthregen_percentage", "0.25",  bit.band(FCVAR_ARCHIVE , FCVAR_REPLICATED , FCVAR_NOTIFY))
CreateConVar("dec_healthregen_enabled", "1",  bit.band(FCVAR_ARCHIVE , FCVAR_REPLICATED , FCVAR_NOTIFY))
CreateConVar("dec_healthregen_dmgpenalty", "15",  bit.band(FCVAR_ARCHIVE , FCVAR_REPLICATED , FCVAR_NOTIFY))
CreateConVar("dec_healthregen_regentime", "2",  bit.band(FCVAR_ARCHIVE , FCVAR_REPLICATED , FCVAR_NOTIFY))
CreateConVar("dec_stamina_drain_sprint_time", "0.6",  bit.band(FCVAR_ARCHIVE , FCVAR_REPLICATED , FCVAR_NOTIFY))
CreateConVar("dec_stamina_drain_sprint_amount", "1",  bit.band(FCVAR_ARCHIVE , FCVAR_REPLICATED , FCVAR_NOTIFY))
CreateConVar("dec_stamina_drain_jump", "5",  bit.band(FCVAR_ARCHIVE , FCVAR_REPLICATED , FCVAR_NOTIFY))
CreateConVar("dec_stamina_drain_ledgepull", "10",  bit.band(FCVAR_ARCHIVE , FCVAR_REPLICATED , FCVAR_NOTIFY))
CreateConVar("dec_stamina_drain_ledgehang_time", "1",  bit.band(FCVAR_ARCHIVE , FCVAR_REPLICATED , FCVAR_NOTIFY))
CreateConVar("dec_stamina_drain_ledgehang_amount", "1",  bit.band(FCVAR_ARCHIVE , FCVAR_REPLICATED , FCVAR_NOTIFY))
CreateConVar("dec_stamina_drain_underwater_time", "0.3",  bit.band(FCVAR_ARCHIVE , FCVAR_REPLICATED , FCVAR_NOTIFY))
CreateConVar("dec_stamina_drain_underwater_amount", "1",  bit.band(FCVAR_ARCHIVE , FCVAR_REPLICATED , FCVAR_NOTIFY))
CreateConVar("dec_stamina_regen_time", "0.2",  bit.band(FCVAR_ARCHIVE , FCVAR_REPLICATED , FCVAR_NOTIFY))
CreateConVar("dec_stamina_regen_amount", "1",  bit.band(FCVAR_ARCHIVE , FCVAR_REPLICATED , FCVAR_NOTIFY))
CreateConVar("dec_pickup_weight", "30",  bit.band(FCVAR_ARCHIVE , FCVAR_REPLICATED , FCVAR_NOTIFY))
CreateConVar("dec_pickup_enabled", "1",  bit.band(FCVAR_ARCHIVE , FCVAR_REPLICATED , FCVAR_NOTIFY))
CreateConVar("dec_propdamage", "0",  bit.band(FCVAR_ARCHIVE , FCVAR_REPLICATED , FCVAR_NOTIFY))
CreateConVar("dec_chat_distance", "256",  bit.band(FCVAR_ARCHIVE , FCVAR_REPLICATED , FCVAR_NOTIFY))
CreateConVar("dec_mic_distance", "256",  bit.band(FCVAR_ARCHIVE , FCVAR_REPLICATED , FCVAR_NOTIFY))
CreateConVar("dec_max_armedkills", "4",  bit.band(FCVAR_ARCHIVE , FCVAR_REPLICATED , FCVAR_NOTIFY))
CreateConVar("dec_bandur", "60",  bit.band(FCVAR_ARCHIVE , FCVAR_REPLICATED , FCVAR_NOTIFY))
CreateConVar("dec_noreturnfirekarmaloss", "0",  bit.band(FCVAR_ARCHIVE , FCVAR_REPLICATED , FCVAR_NOTIFY))
CreateConVar("dec_karmalossmaxdamage", "175",  bit.band(FCVAR_ARCHIVE , FCVAR_REPLICATED , FCVAR_NOTIFY))
CreateConVar("dec_confirm_agent", "100",  bit.band(FCVAR_ARCHIVE , FCVAR_REPLICATED , FCVAR_NOTIFY))
CreateConVar("dec_confirm_vip", "200",  bit.band(FCVAR_ARCHIVE , FCVAR_REPLICATED , FCVAR_NOTIFY))
	...
	for k, v in pairs(player.GetAll()) do
		if v:Alive() then
			...
			if v.ExamTarget then
				...
				if dist <= 256 then
					Dot = v:GetAimVector():DotProduct((HSP - SP):GetNormalized())
					
					if Dot >= 0.7 then
						td = {}
						td.start = SP
						td.endpos = td.start + (HSP - SP):GetNormalized() * 256
						td.filter = v
...
function GM:EntityTakeDamage(ent, dmginfo)
	...
	if attacker:IsPlayer() and IsValid(attacker:GetActiveWeapon()) and isply then
		...
		elseif not ent.IsAgent and not ent.IsVIP then
			ent.DamageDone = ent.DamageDone + dmginfo:GetDamage()
		end
	end
		...
		if attacker:IsValid() and attacker:IsPlayer() then
			...
			elseif attacker.IsVIP then
				...
				if ply.IsAgent then
					...
					if ply.DamageDone > GetConVarNumber("dec_karmalossmaxdamage") then
						attacker:DecreaseKarma(10)
					end
				end
			end
			...
			if dot > 0.6 and ply.IsAgent then
				if CT > ply.PickpocketDelay then
					...
					if dist < 60 then -- didn't want to use another trace, so I'll just distance it instead
						...
						--ply:DoAnimationEvent(ACT_ITEM_PLACE)
					end
				end
			else
...
function PLAYER:DropWep()
	...
	self.Weight = self.Weight - mywep.Size
...
function DEC_UseRadio(ply, com, args)
	CT = CurTime()
...
function DEC_Channel_Join(ply, com, args)
...
function DEC_Channel_Create(ply, com, args)
	if not ply:Alive() or not ply.HasWalkieTalkie then
		return
	end
...
function DEC_Channel_Delete(ply, com, args)
	if not ply:Alive() or not ply.HasWalkieTalkie then
		return
	end
...
function DEC_ConfirmRoleReceival(ply)
...
function DEC_GiveMoney(ply, com, args)
	if not GameStarted or not CanBuy then
		return
	end
