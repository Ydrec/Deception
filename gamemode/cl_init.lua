...
surface.CreateFont( "DefaultBoldShadow", {
	font = "DefaultBold",
	size = 13,
	weight = 400,
	blursize = 0,
...
--cfont("DefaultBold", 12, 400, true, false, "DefaultBoldShadow", true)
surface.CreateFont( "Trebuchet24_AA", {
	font = "Trebuchet24",
	size = 24,
	weight = 500,
	blursize = 0,
...
surface.CreateFont( "Trebuchet64_AA", {
	font = "Trebuchet24",
	size = 64,
	weight = 500,
	blursize = 0,
...
--cfont("Trebuchet24", 24, 500, true, false, "Trebuchet24_AA")
--cfont("Trebuchet24", 64, 500, true, false, "Trebuchet64_AA")
--cfont("DefaultBold", 12, 400, true, false, "DefaultBoldShadow", true)
...
local CT, X, Y, FT, HP, Time, clip, td, trace, cl, H, wep2, dt3, ProgressIncrease, aim, haim, dot, x, y, size, size2, x2, has, stam, amt, ply, wep, dtint2, SC, sizer, EP, SP, par, gp, pos, ang
...
function GM:Think()
	...
	if RoundTime and RoundTime != -1 then
		...
		if StressMusic1:IsPlaying() then
			...
			if timer.IsTimer("StressMusic1") then
...
function GM:HUDPaint()
	...
	if ply:Alive() then
		...
		if IsValid(wep) then
			...
			stext("Current: " .. wep.PrintName or "?", "Trebuchet24_AA", X - 50, Y - 100, Color(255, 255, 255, 255), Color(0, 0, 0, 255), 1, TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER, false)
		...
		if IsValid(tent) then
			...
			if tent:IsPlayer() then
				if PlayerAlpha > 5 then
					...
					if ply.TeamText == "Agent" then -- don't run pointless calculations
