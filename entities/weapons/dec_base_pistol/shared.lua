...
function SWEP:Equip(ply)
	ply.Weight = ply.Weight + self.Size
end
...
function SWEP:Recoil(recoil)
	if ( (game.SinglePlayer() && SERVER) || ( !game.SinglePlayer() && CLIENT && IsFirstTimePredicted() ) ) then
		...
		eyeang.yaw = eyeang.yaw - ((recoil * math.random(-1, 1) * 0.25) * self.Owner.InexperienceRatio)
...
if CLIENT then
	...
	function SWEP:DrawHUD()
		...
		if ( self.Owner == LocalPlayer() && self.Owner:ShouldDrawLocalPlayer() ) then
			...
			tr.mask = bit.band(CONTENTS_SOLID,CONTENTS_MOVEABLE,CONTENTS_MONSTER,CONTENTS_WINDOW,CONTENTS_DEBRIS,CONTENTS_GRATE,CONTENTS_AUX )
